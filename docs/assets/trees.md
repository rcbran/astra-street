...
- Broadleaf additionally uses UV-aware RGB padding (`pad_broadleaf_atlas.py`). Its bake contains opaque white unused regions that the earlier alpha-only fill preserved. All three LODs plus a two-texel margin verified against continuous bilinear support protect 223,742 atlas pixels; 689,857 unused RGB pixels are changed using the nearest mapped opaque non-white color. Every alpha byte and protected source pixel is retained. No leaf tint, UV/geometry change or runtime filtering override is applied. The resulting GLB is 5,702,788 bytes; exact hashes and invariant checks are in `tree_small_02-padding.json`.
