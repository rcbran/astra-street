...
const headless = process.env.ASTRA_HEADLESS === '1';
const port = Number(process.env.ASTRA_CDP_PORT ?? 9224);
const browser = await chromium.launch({
  channel:
    process.env.ASTRA_BROWSER_CHANNEL || (headless ? undefined : 'chrome'),
  headless,
...
if (headless)
  console.log(
    'Use for functional checks; headless timing is not a hardware benchmark.',
  );
