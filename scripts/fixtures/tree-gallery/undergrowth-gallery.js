...
import {
  ...
  fernGeometry,
  shrubGeometry,
} from '../../../src/game/world/undergrowth-geometry.ts';
async function loadTreeAssets() {
  ...
  const species = [grassClumpGeometry, fernGeometry, shrubGeometry].map(
    (create) => {
      const group = new THREE.Group();
      group.add(new THREE.Mesh(create(), material));
      return { id: create.name.replace('Geometry', ''), lods: [group] };
    },
  );
  return {
    species,
